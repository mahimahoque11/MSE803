...
between Happiness and the other features. Activity 1.2 extends Activity 1.1 with
the teacher-taught IQR and Z-score outlier-detection methods.
...
```powershell
python .\week4\activity\activity1.2\happiness_dashboard.py
```
...
Neither the 1.5×IQR method nor the `|Z| > 3` method detected an outlier, so all
20 records were retained. See the dashboard report for the complete analysis,
outlier decision, and interpretation.

![Outlier detection across numeric features](outputs/outlier_detection_boxplots.png)

## Outlier outputs

- `outputs/outlier_detection_summary.csv` - IQR bounds, counts, and maximum
  absolute Z-scores for each numeric feature.
- `outputs/outlier_records.csv` - records flagged by either method; header-only
  in this dataset because no outliers were found.
- `outputs/outlier_reviewed_dataset.csv` - record-level flags and keep/review
  decisions.
- `outputs/outlier_detection_boxplots.png` - static outlier visualisation.
- `outputs/outlier_detection.html` - interactive outlier visualisation.
